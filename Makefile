...
Xaw3d::
	...
	do \
		echo Making "in $(CURRENT_DIR)/$$i..."; \
		$(MAKE) -C $$i $(MFLAGS) $(PARALLELMFLAGS)  Xaw3d; \
	done
...
LOCAL_LIBRARIES = Widgets/libaxe.a ${TABLE_LIBS} ${EXTN_LIBS} ${XAW3D_LIBS}
        DEPLIBS = Widgets/libaxe.a $(DEPXAWLIB) $(DEPXMULIB) $(DEPXTOOLLIB) $(DEPXPMLIB) $(EXTRAXAWCLIENTDEPLIBS) $(DEPXLIB)
...
install:: axe
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  axe $(DESTDIR)${Bindir}/axe

all:: axe.$(MANNEWSUFFIX)

axe.$(MANNEWSUFFIX): axe.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname axe` && \
	   $(LN) `basename axe.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < axe.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) axe.$(MANNEWSUFFIX)

install.man:: axe.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) axe.$(MANNEWSUFFIX) $(DESTDIR)${Mandir}/axe.$(MANSUFFIX)
...
install:: Axe.ad
	@if [ -d $(DESTDIR)${Appdir} ]; then \
		...
		if [ -h $(DESTDIR)${Appdir} ]; then \
			(set -x; rm -f $(DESTDIR)${Appdir}); \
		fi; \
		(set -x; $(MKDIRHIER) $(DESTDIR)${Appdir}); \
	fi
	$(INSTALL) -c $(INSTAPPFLAGS) Axe.ad $(DESTDIR)${Appdir}/Axe

install:: axe.tcl
	@if [ -d $(DESTDIR)${AxeLibdir} ]; then \
		set +x; \
	else \
		if [ -h $(DESTDIR)${AxeLibdir} ]; then \
			(set -x; rm -f $(DESTDIR)${AxeLibdir}); \
		fi; \
		(set -x; $(MKDIRHIER) $(DESTDIR)${AxeLibdir}); \
	fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) axe.tcl $(DESTDIR)${AxeLibdir}/axe.tcl

axe.o:	version.h
	$(RM) $@
	 	$(CC) -c $(CFLAGS) -DDUMMY $*.c

all:: coaxe

coaxe: coaxe.o Widgets/libaxe.a
	$(RM) $@
	$(CCLINK) -o $@ $(LDOPTIONS) coaxe.o Widgets/libaxe.a $(LDLIBS) ${XLIB} $(EXTRA_LOAD_FLAGS)
...
install:: coaxe
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  coaxe $(DESTDIR)${Bindir}/coaxe
install::
	if [ -f ${Bindir}/faxe ]; then set +x; \
	else (cd ${Bindir}; set -x; $(LN) coaxe faxe); fi
	if [ -f ${Bindir}/poleaxe ]; then set +x; \
	else (cd ${Bindir}; set -x; $(LN) coaxe poleaxe); fi
...
Axe.ad:	Axe.ad.sed version.h Axe.tmpl
	sed -e "s/VERSION/`sed -e 's/.*VERSION \(.*\)/\1/;q' <version.h`/" \
	    -e "s+EXTENSION+\/\*\*\/+"				       \
            -e "s+AXELIBDIR+${AxeLibdir}+" <Axe.ad.sed >Axe.ad
...
Xaw3d::
	$(RM) axe
	@if [ -d X11/Xaw3d/X11 ]; then \
		set +x; \
	else \
		if [ -h X11/Xaw3d/X11 ]; then \
			(set -x; rm -f X11/Xaw3d/X11); \
		fi; \
		(set -x; $(MKDIRHIER) X11/Xaw3d/X11); \
	fi
	@(set -x; cd X11/Xaw3d/X11; for i in Xaw Xaw3d; do \
		$(RM) $$i; \
		ln -s /usr/include/X11/Xaw3d $$i; \
	done)
...
# DO NOT DELETE
axe.o: axe.c /usr/include/stdc-predef.h /usr/include/X11/IntrinsicP.h \
 ...
 /usr/include/X11/Xatom.h /usr/include/X11/Shell.h \
 /usr/include/X11/SM/SMlib.h /usr/include/X11/SM/SM.h \
 /usr/include/X11/ICE/ICElib.h /usr/include/X11/ICE/ICE.h \
 Widgets/X11/Xp/Table.h Widgets/X11/Xp/COPY /usr/include/X11/Xaw/Form.h \
 /usr/include/X11/Xaw/Label.h /usr/include/X11/Xaw/Simple.h \
 /usr/include/X11/Xmu/Converters.h /usr/include/X11/Xaw/Command.h \
 Widgets/AxeIntrinsic.h Widgets/AxeWindow.h Widgets/AxeEditor.h \
 /usr/include/X11/Xaw/Paned.h Widgets/AxeMenuBtn.h \
 /usr/include/X11/Xaw/MenuButton.h Widgets/Confirmer.h \
 Widgets/FileNomWin.h Widgets/FileNom.h version.h Widgets/axeLogo.xbm \
 ...
 /usr/include/x86_64-linux-gnu/bits/stdlib-float.h Widgets/util.h
